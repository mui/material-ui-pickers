...
Pickers are originally written in typescript, so type definetions are provided with the code without any additional requirements.
...
We don't really know which adapter you will pass to the `<LocalizationProvider />`. So any picker component is generic by default.
