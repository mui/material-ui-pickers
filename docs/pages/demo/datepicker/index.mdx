...
The default `Datepicker` component is responsive, which means that `Mobile` or `Desktop` mode will be rendered according to pointer support of the device. If the pointer is touch, it uses the `Mobile` mode. If the pointer is mouse, it uses the `Desktop` mode.
