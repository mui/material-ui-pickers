...
Material-UI pickers rely on the date management library when the date should be parsed. Any other prop-types that accepts dates (e.g. `minDate`, `maxDate`) it can take string, number, Date object and so on.
