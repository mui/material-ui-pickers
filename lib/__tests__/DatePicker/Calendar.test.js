...
describe('Calendar - disabled selected date on mount', () => {
  ...
  it('Should call onSelectMonth', () => {
    const { onSelectMonth } = component.instance().props;
    const value = {};

    component.instance().handleChangeMonth(value);

    expect(onSelectMonth).toHaveBeenCalledWith(value);
  });
});
